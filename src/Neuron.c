...
float neuron_forward(Neuron* neuron, float* inputs){
    ...
    // Aplikácia aktivácie (softmax sa aplikuje na úrovni vrstvy)
    if(neuron->activation_type == ACTIVATION_RELU){
        sum = relu(sum);
    } else if(neuron->activation_type == ACTIVATION_SIGMOID){
        sum = sigmoid(sum);
    }
...
void neuron_backward(Neuron* neuron, float* inputs, float learning_rate){
    ...
    if(neuron->activation_type == ACTIVATION_RELU){
        ...
        gradient = neuron->delta * sigmoid_derivative(neuron->sum);
