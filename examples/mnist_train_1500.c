...
// Network architecture - medium size for intermediate dataset
#define INPUT_SIZE 784      // 28x28 pixels
#define HIDDEN1_SIZE 200    // Medium first hidden layer
#define HIDDEN2_SIZE 100    // Medium second hidden layer
#define OUTPUT_SIZE 10      // 10 digits (0-9)

// Training hyperparameters - adjusted for medium dataset
#define MAX_TRAIN_SAMPLES 1500  // Limit training samples
#define NUM_EPOCHS 80       // Moderate number of epochs
#define BATCH_SIZE 48       // Medium batch size
#define INITIAL_LEARNING_RATE 0.001f
#define MIN_LEARNING_RATE 0.0001f
#define VALIDATION_RATIO 0.15f // Medium validation ratio

// Early stopping parameters
#define PATIENCE 8          // Medium patience
#define MIN_DELTA 0.0005f   // Medium minimum delta

float adjust_learning_rate(float current_lr, int epoch){
    // Moderate decay for medium dataset
...
int main(){
    ...
    // Limit training data to 1500 samples
    ...
    for(int epoch = 0; epoch < NUM_EPOCHS; epoch++){
        ...
        printf("\rEpoch %d/%d - Train Loss: %.4f, Val Loss: %.4f, Val Acc: %.2f%%, LR: %.6f\n",
        ...
        if(val_loss < best_val_loss - MIN_DELTA){
            ...
            if(model_save(network, "models/mnist_model_1500.bin") == 0){
                printf("1500-image model saved successfully\n");
            }
        } else{
            patience_counter++;
    ...
    printf("\n1500-image model training completed!\n");
    return 0;
} 