...
// Architektúra siete - menšia pre obmedzené dáta
#define INPUT_SIZE 784      // 28x28 pixelov
#define HIDDEN1_SIZE 128    // Prvá skrytá vrstva (ReLU)
#define HIDDEN2_SIZE 64     // Druhá skrytá vrstva (Sigmoid)
#define OUTPUT_SIZE 10      // 10 číslic (0-9) so Softmax

// Hyperparametre tréningu - upravené pre malý dataset
#define MAX_TRAIN_SAMPLES 300  // Limit trénovacích vzoriek
#define NUM_EPOCHS 100      // Viac epoch pre malý dataset
#define BATCH_SIZE 32       // Menšia veľkosť batch
#define INITIAL_LEARNING_RATE 0.001f
#define MIN_LEARNING_RATE 0.0001f
#define VALIDATION_RATIO 0.2f // Vyšší pomer validácie

// Výber optimalizátora: 1 pre Adam, 0 pre SGD
#define USE_ADAM 1

// Parametre early stopping
#define PATIENCE 10         // Viac trpezlivosti pre malý dataset
#define MIN_DELTA 0.001f    // Väčšia minimálna delta

float adjust_learning_rate(float current_lr, int epoch){
    // Pomalší pokles pre malý dataset
...
int main(){
    ...
    // Obmedzenie trénovacích dát na 300 vzoriek
    ...
    NeuralNetwork* best_network = network_load("models/mnist_model_300.bin");
    if(!best_network){
